...
export class ObiInteger extends ObiBase {
    ...
    encode(value) {
        ...
        if (newValue > this.maxValue) {
            throw new EncodeError(`Value ${value} exceeds maximum size for ${this.sizeInBytes * 8} bits`);
        }
    ...
    decode(buff) {
        if (buff.length < this.sizeInBytes) {
            throw new DecodeError(`Not enough bytes. Expected ${this.sizeInBytes}, got ${buff.length}`);
        }
        let value = BigInt(0);
    ...
    parseSchema(schema) {
        ...
        try {
            ...
            if (error instanceof Error) {
                throw new SchemaError(`Failed to parse struct schema: ${error.message}`);
            }
...
export class ObiString extends ObiBase {
    ...
    decode(buff) {
        ...
        if (remaining.length < stringLength) {
            throw new DecodeError(`Not enough bytes to decode string. Expected ${stringLength}, got ${remaining.length}`);
        }
        const value = remaining.slice(0, stringLength).toString("utf8");
        return [value, remaining.slice(stringLength)];
    }
}
export class ObiBytes extends ObiBase {
    static REGEX = /^bytes$/;
    lengthEncoder = new ObiInteger("u32");
    encode(value) {
        return Buffer.concat([
    ...
    decode(buff) {
        ...
        if (remaining.length < bytesLength) {
            throw new DecodeError(`Not enough bytes to decode bytes. Expected ${bytesLength}, got ${remaining.length}`);
        }
        return [
...
export class Obi {
    ...
    encodeInput(value) {
        try {
            ...
            throw new EncodeError(`Failed to encode input: ${error.message}`);
        }
    }
    decodeInput(buff) {
    ...
    encodeOutput(value) {
        try {
            ...
            throw new EncodeError(`Failed to encode output: ${error.message}`);
        }
    }
    decodeOutput(buff) {
